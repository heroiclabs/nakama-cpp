...
template <typename R> R getFromFuture(std::future<R> f, Satori::SClientPtr client) {
  if (!f.valid()) {
    return R();
  }
...
int main(int argc, char** argv) {
  try {
    ...
    Satori::SSessionPtr session1 = getFromFuture(
        client->authenticateAsync("11111111-1111-0000-0000-000000000000", {}, {{"pushTokenIos", "foo"}}), client);
    Satori::SSessionPtr session2 = getFromFuture(
        client->authenticateAsync("22222222-2222-0000-0000-000000000000", {}, {{"pushTokenAndroid", "bar"}}), client);
    Satori::SSessionPtr session3 =
