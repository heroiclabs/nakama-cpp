...
void GrpcClient::disconnect() { _cq.Shutdown(); }
...
void GrpcClient::setBasicAuth(ReqContext* ctx) { ctx->context.AddMetadata("authorization", _basicAuthMetadata); }
...
void GrpcClient::unlinkFacebook(
    NSessionPtr session,
    const std::string& accessToken,
    std::function<void()> successCallback,
    ErrorCallback errorCallback) {
...
void GrpcClient::unlinkGameCenter(
    NSessionPtr session,
    const std::string& playerId,
    const std::string& bundleId,
    NTimestamp timestampSeconds,
...
void GrpcClient::importFacebookFriends(
  ...
  req.mutable_account()->set_token(token);
  if (reset)
    req.mutable_reset()->set_value(*reset);
...
void GrpcClient::updateAccount(
  ...
  if (username)
    req.mutable_username()->set_value(*username);
  if (displayName)
    req.mutable_display_name()->set_value(*displayName);
  if (avatarUrl)
    req.mutable_avatar_url()->set_value(*avatarUrl);
  if (langTag)
    req.mutable_lang_tag()->set_value(*langTag);
  if (location)
    req.mutable_location()->set_value(*location);
  if (timezone)
    req.mutable_timezone()->set_value(*timezone);
...
void GrpcClient::listFriends(
  ...
  if (state)
    req.mutable_state()->set_value((google::protobuf::int32)*state);
  if (!cursor.empty())
...
void GrpcClient::kickGroupUsers(
    NSessionPtr session,
    const std::string& groupId,
    const std::vector<std::string>& ids,
    std::function<void()> successCallback,
...
void GrpcClient::promoteGroupUsers(
    NSessionPtr session,
    const std::string& groupId,
    const std::vector<std::string>& ids,
    std::function<void()> successCallback,
...
void GrpcClient::updateGroup(
  ...
  if (name)
    req.mutable_name()->set_value(*name);
  if (description)
    req.mutable_description()->set_value(*description);
  if (avatarUrl)
    req.mutable_avatar_url()->set_value(*avatarUrl);
  if (langTag)
    req.mutable_lang_tag()->set_value(*langTag);
  if (open)
    req.mutable_open()->set_value(*open);
...
void GrpcClient::listLeaderboardRecordsAroundOwner(
    NSessionPtr session,
    const std::string& leaderboardId,
...
void GrpcClient::writeLeaderboardRecord(
  ...
  if (subscore)
    req.mutable_record()->set_subscore(*subscore);
  if (metadata)
    req.mutable_record()->set_metadata(*metadata);
...
void GrpcClient::deleteLeaderboardRecord(
    NSessionPtr session,
    const std::string& leaderboardId,
    std::function<void()> successCallback,
    ErrorCallback errorCallback) {
...
void GrpcClient::listMatches(
  ...
  if (min_size)
    req.mutable_min_size()->set_value(*min_size);
  if (max_size)
    req.mutable_max_size()->set_value(*max_size);
  if (limit)
    req.mutable_limit()->set_value(*limit);
  if (label)
    req.mutable_label()->set_value(*label);
  if (query)
    req.mutable_query()->set_value(*query);
  if (authoritative)
    req.mutable_authoritative()->set_value(*authoritative);
...
void GrpcClient::listNotifications(
  ...
  if (limit)
    req.mutable_limit()->set_value(*limit);
  if (cacheableCursor)
    req.set_cacheable_cursor(*cacheableCursor);
...
void GrpcClient::deleteNotifications(
    NSessionPtr session,
...
void GrpcClient::listChannelMessages(
  ...
  if (cursor)
    req.set_cursor(*cursor);
  if (forward)
    req.mutable_forward()->set_value(*forward);
...
void GrpcClient::listTournaments(
  ...
  if (categoryStart)
    req.mutable_category_start()->set_value(*categoryStart);
  if (categoryEnd)
    req.mutable_category_end()->set_value(*categoryEnd);
  if (startTime)
    req.mutable_start_time()->set_value(*startTime);
  if (endTime)
    req.mutable_end_time()->set_value(*endTime);
  if (limit)
    req.mutable_limit()->set_value(*limit);
  if (cursor)
    req.set_cursor(*cursor);
...
void GrpcClient::joinTournament(
    NSessionPtr session,
    const std::string& tournamentId,
    std::function<void()> successCallback,
    ErrorCallback errorCallback) {
...
void GrpcClient::deleteStorageObjects(
    NSessionPtr session,
