...
release_libs_dir = os.path.abspath('../../release/nakama-cpp-sdk/libs/' + TARGET_PLATFORM)
...
if BUILD_NAKAMA_STATIC:
    ...
    for arch in arch_list:
        call(['python', 'build_tvos.py', arch])
    ...
    if USE_CPPREST:
        ...
        boost_libs_path = os.path.abspath('../../third_party/cpprestsdk/Build_tvOS/boost/lib')
...
if BUILD_NAKAMA_SHARED:
    # dynamic libs
    release_libs_dir = os.path.abspath('../../release/nakama-cpp-sdk/shared-libs/' + TARGET_PLATFORM)

    nakama_cpp_libs = []

    for arch in arch_list:
        call(['python', 'build_tvos.py', '--dylib', arch])
        
        build_arch_dir = build_dir + arch
        dylib_in_build = build_arch_dir + '/src/libnakama-cpp.dylib'

        if TVOS_UNIVERSAL_SHARED_LIB:
