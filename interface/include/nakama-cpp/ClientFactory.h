...
struct NClientParameters {
  /// The key used to authenticate with the server without a session. Defaults
  /// to "defaultkey".
  ...
  /// Set connection strings to use the secure mode with the server. Defaults to
  /// false. The server must be configured to make use of this option. With
  /// HTTP, GRPC, and WebSockets the server must be configured with an SSL
  /// certificate or use a load balancer which performs SSL termination. For
  /// rUDP you must configure the server to expose it's IP address so it can be
  /// bundled within session tokens. See the server documentation for more
  /// information.
  bool ssl = false;
 ...
 *
 * @param parameters the client parameters
 * @param httpTransport optional, the HTTP client. If not set then default HTTP
 * transport will be used.
 */
NAKAMA_API NClientPtr
createRestClient(const NClientParameters &parameters,
                 NHttpTransportPtr httpTransport = nullptr);

/**
 * Creates default HTTP transport using C++ REST SDK.
 */
#ifdef HAVE_DEFAULT_TRANSPORT_FACTORY
NAKAMA_API NHttpTransportPtr
