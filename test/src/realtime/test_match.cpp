...
void test_rt_sendmatchdata_throughput() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    while (testMessages[i].size() < 1000000) {
      testMessages[i] +=
  ...
  test1.rtClient->sendMatchDataAsync(matched->matchId,101, testMessages[1]).get();
  test1.rtClient->sendMatchDataAsync(matched->matchId,101, testMessages[2]).get();
