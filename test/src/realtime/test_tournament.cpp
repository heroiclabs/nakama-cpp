...
void test_tournament() {
  ...
  document.AddMember(
      "description", "Dash past your opponents for high scores and big rewards!", document.GetAllocator());
  ...
  if (responseDocument.Parse(rpc.payload).HasParseError()) {
    ...
    if (jsonTournamentId.IsString()) {
      ...
      auto successCallback = [&, tournamentId]() {
        ...
        test.rtClient->rpc(
            "clientrpc.delete_tournament", "{\"tournament_id\":\"" + tournamentId + "\"}", successCallback);
      };

      test.client->joinTournament(session, tournamentId, successCallback);
    } else {
      test.stopTest();
    }
  }
