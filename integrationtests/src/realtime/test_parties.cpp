...
void test_rt_create_party() {
  ...
  auto partyJoinCallback = [&partyJoinPromise](const NParty& party) { partyJoinPromise.set_value(); };

  test2.listener.setPartyCallback(partyJoinCallback);

  test1.listener.setPartyDataCallback([&partyDataPromise](const NPartyData& data) { partyDataPromise.set_value(); });
