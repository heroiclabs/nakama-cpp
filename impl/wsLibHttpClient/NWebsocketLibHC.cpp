...
NWebsocketLibHC::NWebsocketLibHC(XTaskQueueHandle q):
    m_queue(q, &XTaskQueueCloseHandle),
    m_ws(nullptr, &HCWebSocketCloseHandle),
    m_is_binary(false),
    _activityTimeoutMs(0),
...
void NWebsocketLibHC::connect(const std::string &url, NRtTransportType type) {
    {
        ...
        auto hr = HCWebSocketCreate(&ws, &ws_on_text_msg, &ws_on_binary_msg, &ws_on_close, this);
        if (FAILED(hr)) {
    ...
    asyncBlock->callback = [](XAsyncBlock *ab) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (self->m_ws /*&& self->m_ws.get() == result.websocket*/) {
                ...
                HC_TRACE_WARNING(wsTransportLibHC, "User requested disconnect, while connection was in progress. Ignoring connection result.");
            }
        } else {
...
void NWebsocketLibHC::setActivityTimeout(uint32_t timeoutMs)
{
    _activityTimeoutMs = timeoutMs;
}

uint32_t NWebsocketLibHC::getActivityTimeout() const
{
...
void NWebsocketLibHC::tick()
{
    ...
    if (isConnected() && _activityTimeoutMs > 0 && getUnixTimestampMs() - _lastReceivedMessageTimeMs >= _activityTimeoutMs)
    {
        disconnect();
    }
}
...
bool NWebsocketLibHC::send(const NBytes& data) {
    ...
    asyncBlock->callback = [](XAsyncBlock *ab) {
        ...
        if (FAILED(result.errorCode) || FAILED(hr)) {
            HC_TRACE_ERROR(wsTransportLibHC, "Send error: hr=0x%08x ws.errorCode=0x%08x platformErr=0x%08x",
                           hr, result.errorCode, result.platformErrorCode);
            static_cast<NWebsocketLibHC*>(asyncBlock->context)->fireOnError("Error sending msg");
        }
    };
    HRESULT hr;
    if (m_is_binary) {
        hr = HCWebSocketSendBinaryMessageAsync(m_ws.get(),
               reinterpret_cast<const uint8_t *>(data.data()), uint32_t(data.size()), asyncBlock);
    } else {
    ...
    if (auto lock = g_registry.lockKey(self)) {
        ...
        XTaskQueueSubmitCallback(self->m_queue.get(), XTaskQueuePort::Completion, ctx, [](void *context, bool) {
            ...
            if (self->m_ws && self->m_ws.get() == ws) {
                ...
                HC_TRACE_INFORMATION(wsTransportLibHC, "ws_on_close->tick: ignoring close for use-initiated disconnect on %p", ws);
            }
        });
    } else {
