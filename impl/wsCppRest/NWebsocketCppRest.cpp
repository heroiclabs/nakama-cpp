...
NWebsocketCppRest::~NWebsocketCppRest() { disconnect(); }

void NWebsocketCppRest::setActivityTimeout(uint32_t timeoutMs) { _activityTimeoutMs = timeoutMs; }

uint32_t NWebsocketCppRest::getActivityTimeout() const { return _activityTimeoutMs; }
...
void NWebsocketCppRest::connect(const std::string& url, NRtTransportType type) {
  try {
    ...
    _wsClient->set_close_handler(
        std::bind(
            &NWebsocketCppRest::onClosed, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void NWebsocketCppRest::disconnect(web::websockets::client::websocket_close_status status, const std::string& reason) {
  ...
  _wsClient->set_close_handler(
      [](web::web_sockets::client::websocket_close_status, const utility::string_t&, const std::error_code&) {});
...
void NWebsocketCppRest::onOpened() {
  ...
  executeInUserThread([this]() { fireOnConnected(); });
}

// will be executed from internal thread of WsClient
void NWebsocketCppRest::onClosed(
  ...
  executeInUserThread([this, disconnectInfo]() { fireOnDisconnected(*disconnectInfo); });
}
  ...
  try {
    ...
    switch (msg.message_type()) {
      ...
      case web::websockets::client::websocket_message_type::ping: {
        executeInUserThread([]() { NLOG_DEBUG("ping"); });
        break;
      }

      case web::websockets::client::websocket_message_type::pong: {
        executeInUserThread([]() { NLOG_DEBUG("pong"); });
        break;
      }
...
void NWebsocketCppRest::addErrorEvent(std::string&& err) {
  ...
  *errPtr = std::move(err);
  ...
  executeInUserThread([this, errPtr]() { fireOnError(*errPtr); });
}
